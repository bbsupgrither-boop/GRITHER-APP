...
export function ActivityCard({ leaderboard = [], activeBattles = [], onNavigate }: ActivityCardProps) {
  ...
  // РџСЂРµРѕР±СЂР°Р·СѓРµРј Р°РєС‚РёРІРЅС‹Рµ Р±Р°С‚С‚Р»С‹ РІ РЅСѓР¶РЅС‹Р№ С„РѕСЂРјР°С‚
  const transformedBattles: BattleEntry[] = activeBattles.map(battle => ({
    id: parseInt(battle.id),
    participantA: battle.participants.length > 0 ? battle.participants[0].user.name : 'РЈС‡Р°СЃС‚РЅРёРє Рђ',
    participantB: battle.participants.length > 1 ? battle.participants[1].user.name : 'РЈС‡Р°СЃС‚РЅРёРє Р‘',
    status: battle.status === 'active' ? 'active' as const : 'completed' as const
  }));

  // Mock РґР°РЅРЅС‹Рµ РґР»СЏ РґРµРјРѕРЅСЃС‚СЂР°С†РёРё (РµСЃР»Рё РЅРµС‚ Р°РєС‚РёРІРЅС‹С… Р±Р°С‚С‚Р»РѕРІ)
  const mockBattles: BattleEntry[] = transformedBattles.length > 0 ? transformedBattles : [
    { id: 1, participantA: 'РђР»РµРєСЃРµР№', participantB: 'РњР°СЂРёСЏ', status: 'active' },
    { id: 2, participantA: 'Р”РјРёС‚СЂРёР№', participantB: 'РђРЅРЅР°', status: 'completed' },
    { id: 3, participantA: 'РЎРµСЂРіРµР№', participantB: 'Р•Р»РµРЅР°', status: 'cancelled' },
  ];

  const mockHistory: HistoryEntry[] = [
    { id: 1, participantA: 'РРІР°РЅ', participantB: 'РџРµС‚СЂ', status: 'completed', date: '11.09' },
    { id: 2, participantA: 'РћР»СЊРіР°', participantB: 'РљР°С‚СЏ', status: 'completed', date: '10.09' },
    { id: 3, participantA: 'РђРЅС‚РѕРЅ', participantB: 'РџР°РІРµР»', status: 'cancelled', date: '09.09' },
  ];

  const tabs = [
    { id: 'battles' as const, label: 'Р‘Р°С‚С‚Р»С‹' },
    { id: 'leaderboard' as const, label: 'Р›РёРґРµСЂР±РѕСЂРґ' },
    { id: 'history' as const, label: 'РСЃС‚РѕСЂРёСЏ' }
  ];
  ...
  const renderHistoryContent = () => {
    ...
    if (history.length === 0) {
      return (
        <div className="flex items-center justify-center py-8">
          <p className="text-muted-foreground text-center text-sm">
            РСЃС‚РѕСЂРёСЏ РѕС‚СЃСѓС‚СЃС‚РІСѓРµС‚
          </p>
        </div>
      );
    }
  ...
  const handleHistoryClick = () => {
    // РћС‚РєСЂС‹С‚СЊ РёСЃС‚РѕСЂРёСЋ Р·Р° РІСЃРµ РґРЅРё
    console.log('РћС‚РєСЂС‹С‚СЊ РїРѕР»РЅСѓСЋ РёСЃС‚РѕСЂРёСЋ');
  };
  ...
  return (
    <div className="glass-card rounded-2xl p-4 apple-shadow">
      ...
      <div className="relative flex items-center justify-center mb-4">
        ...
        <h2 className="font-medium text-foreground">РђРєС‚РёРІРЅРѕСЃС‚СЊ РґРЅСЏ</h2>
      ...
      {/* РЎРµРіРјРµРЅС‚РЅС‹Р№ РїРµСЂРµРєР»СЋС‡Р°С‚РµР»СЊ */}
      ...
      {/* РџРµСЂРµРєР»СЋС‡Р°С‚РµР»СЊ РґРЅСЏ */}
